...
$fatpacked{"apps/java/peoplesoft/jmx/mode/queuelength.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'APPS_JAVA_PEOPLESOFT_JMX_MODE_QUEUELENGTH';
  #
...
$fatpacked{"apps/java/peoplesoft/jmx/mode/sessions.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'APPS_JAVA_PEOPLESOFT_JMX_MODE_SESSIONS';
  #
  # Copyright 2019 Centreon (http://www.centreon.com/)
  #
  # Centreon is a full-fledged industry-strength solution that meets
  ...
  package apps::java::peoplesoft::jmx::mode::sessions;
  ...
  sub run {
      ...
      $self->{request} = [
           { mbean => $mbean, attributes => [ { name => 'OpenSessionsCurrentCount' } ] },
      ];
      my $result = $self->{connector}->get_attributes(request => $self->{request}, nothing_quit => 1);
      
      my $exit = $self->{perfdata}->threshold_check(value => $result->{$mbean}->{OpenSessionsCurrentCount},
                                                    threshold => [ { label => 'critical', exit_litteral => 'critical' }, { label => 'warning', exit_litteral => 'warning' } ]);
  
      $self->{output}->perfdata_add(label => 'sessions',
                                    value => $result->{$mbean}->{OpenSessionsCurrentCount},
                                    warning => $self->{perfdata}->get_perfdata_for_output(label => 'warning'),
                                    critical => $self->{perfdata}->get_perfdata_for_output(label => 'critical'),
                                    min => 0);
  
      $self->{output}->output_add(severity => $exit,
                                  short_msg => sprintf("Sessions Count : %s", $result->{$mbean}->{OpenSessionsCurrentCount}));
      $self->{output}->display();
      $self->{output}->exit();
  }
...
$fatpacked{"apps/java/peoplesoft/jmx/plugin.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'APPS_JAVA_PEOPLESOFT_JMX_PLUGIN';
  #
  # Copyright 2019 Centreon (http://www.centreon.com/)
  #
  # Centreon is a full-fledged industry-strength solution that meets
  ...
  package apps::java::peoplesoft::jmx::plugin;
  ...
  Check PeopleSoft in JMX. Need Jolokia agent.
