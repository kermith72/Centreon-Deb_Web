...
$fatpacked{"storage/oracle/zs/snmp/mode/components/module.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'STORAGE_ORACLE_ZS_SNMP_MODE_COMPONENTS_MODULE';
  #
  # Copyright 2019 Centreon (http://www.centreon.com/)
  #
  # Centreon is a full-fledged industry-strength solution that meets
  ...
  package storage::oracle::zs::snmp::mode::components::module;
  
  use strict;
  use warnings;
  
  my $map_status = { 1 => 'other', 2 => 'active', 3 => 'failed'};
  
  my $mapping = {
      sunFmModuleName     => { oid => '.1.3.6.1.4.1.42.2.195.1.3.1.2' },
      sunFmModuleStatus   => { oid => '.1.3.6.1.4.1.42.2.195.1.3.1.4', map => $map_status },
  };
  my $oid_sunFmModuleEntry = '.1.3.6.1.4.1.42.2.195.1.3.1';
  
  sub load {
      my ($self) = @_;
      
      push @{$self->{request}}, { oid => $oid_sunFmModuleEntry };
  }
  ...
  sub check {
      ...
      foreach my $oid ($self->{snmp}->oid_lex_sort(keys %{$self->{results}->{$oid_sunFmModuleEntry}})) {
          next if ($oid !~ /^$mapping->{sunFmModuleName}->{oid}\.(.*)$/);
          my $instance = $1;
          my $result = $self->{snmp}->map_instance(mapping => $mapping, results => $self->{results}->{$oid_sunFmModuleEntry}, instance => $instance);
          ...
          $self->{output}->output_add(long_msg => sprintf("module '%s' status is '%s' [instance: %s].",
                                      $result->{sunFmModuleName}, $result->{sunFmModuleStatus},
                                      $instance
                                      ));
...
$fatpacked{"storage/oracle/zs/snmp/mode/hardware.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'STORAGE_ORACLE_ZS_SNMP_MODE_HARDWARE';
  #
  # Copyright 2019 Centreon (http://www.centreon.com/)
  #
  # Centreon is a full-fledged industry-strength solution that meets
  ...
  sub set_system {
      ...
      $self->{regexp_threshold_overload_check_section_option} = '^(module)$';
...
$fatpacked{"storage/oracle/zs/snmp/mode/listshares.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'STORAGE_ORACLE_ZS_SNMP_MODE_LISTSHARES';
  #
  # Copyright 2019 Centreon (http://www.centreon.com/)
  #
  # Centreon is a full-fledged industry-strength solution that meets
...
$fatpacked{"storage/oracle/zs/snmp/mode/shareusage.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'STORAGE_ORACLE_ZS_SNMP_MODE_SHAREUSAGE';
  #
  # Copyright 2019 Centreon (http://www.centreon.com/)
  #
  # Centreon is a full-fledged industry-strength solution that meets
...
$fatpacked{"storage/oracle/zs/snmp/plugin.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'STORAGE_ORACLE_ZS_SNMP_PLUGIN';
  #
  # Copyright 2019 Centreon (http://www.centreon.com/)
  #
  # Centreon is a full-fledged industry-strength solution that meets
