...
$fatpacked{"network/cisco/callmanager/snmp/mode/ccmusage.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'NETWORK_CISCO_CALLMANAGER_SNMP_MODE_CCMUSAGE';
  #
  # Copyright 2019 Centreon (http://www.centreon.com/)
  #
  # Centreon is a full-fledged industry-strength solution that meets
  ...
  sub set_counters {
      ...
      foreach (@map) {
          ...
          push @{$self->{maps_counters}->{global}}, { label => $_->[0], set => {
                  ...
                  output_template => $_->[1],
                  perfdatas => [
                      { label => $label, value => $_->[2] . '_absolute', template => '%s', min => 0 },
                  ],
              }
          },
      }
...
$fatpacked{"network/cisco/callmanager/snmp/mode/ctiusage.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'NETWORK_CISCO_CALLMANAGER_SNMP_MODE_CTIUSAGE';
  #
  # Copyright 2019 Centreon (http://www.centreon.com/)
  #
  # Centreon is a full-fledged industry-strength solution that meets
  ...
  sub set_counters {
      ...
      foreach (@map) {
          push @{$self->{maps_counters}->{global}}, { label => $_->[0], nlabel => 'cti.devices.total.' . $_->[2] . '.count', set => {
                  ...
                  output_template => $_->[1],
                  perfdatas => [
                      { value => $_->[2] . '_absolute', template => '%s', min => 0 },
                  ],
              }
          },
      }
...
$fatpacked{"network/cisco/callmanager/snmp/mode/gatewayusage.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'NETWORK_CISCO_CALLMANAGER_SNMP_MODE_GATEWAYUSAGE';
  #
  # Copyright 2019 Centreon (http://www.centreon.com/)
  #
  # Centreon is a full-fledged industry-strength solution that meets
...
$fatpacked{"network/cisco/callmanager/snmp/mode/mediadeviceusage.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'NETWORK_CISCO_CALLMANAGER_SNMP_MODE_MEDIADEVICEUSAGE';
  #
  # Copyright 2019 Centreon (http://www.centreon.com/)
  #
  # Centreon is a full-fledged industry-strength solution that meets
  ...
  sub custom_status_calc {
      ...
      $self->{result_values}->{status} = $options{new_datas}->{$self->{instance} . '_ccmMediaDeviceStatus'};
  ...
  sub set_counters {
      ...
      $self->{maps_counters}->{md} = [
          { label => 'status', threshold => 0, set => {
                  key_values => [ { name => 'ccmMediaDeviceStatus' }, { name => 'ccmMediaDeviceName' } ],
                  closure_custom_calc => $self->can('custom_status_calc'),
                  closure_custom_output => $self->can('custom_status_output'),
                  closure_custom_perfdata => sub { return 0; },
                  closure_custom_threshold_check => \&catalog_status_threshold,
  ...
  my $mapping = {
      ccmMediaDeviceName      => { oid => '.1.3.6.1.4.1.9.9.156.1.6.1.1.2' },
      ccmMediaDeviceStatus    => { oid => '.1.3.6.1.4.1.9.9.156.1.6.1.1.5', map => \%mapping_status },
  };
  ...
  sub manage_selection {
      ...
      my $snmp_result = $options{snmp}->get_table(oid => $oid_ccmMediaDeviceEntry, start => $mapping->{ccmMediaDeviceName}->{oid}, end => $mapping->{ccmMediaDeviceStatus}->{oid}, nothing_quit => 1);
      
      $self->{md} = {};
      $self->{global} = { unknown => 0, registered => 0, unregistered => 0, rejected => 0, partiallyregistered => 0 };
      foreach my $oid (keys %$snmp_result) {
          next if ($oid !~ /^$mapping->{ccmMediaDeviceStatus}->{oid}\.(.*)/);
          my $instance = $1;
          my $result = $options{snmp}->map_instance(mapping => $mapping, results => $snmp_result, instance => $instance);
          
          $self->{md}->{$instance} = { %$result };
          $self->{global}->{$result->{ccmMediaDeviceStatus}}++;
      }
  }
...
$fatpacked{"network/cisco/callmanager/snmp/mode/phoneusage.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'NETWORK_CISCO_CALLMANAGER_SNMP_MODE_PHONEUSAGE';
  #
  # Copyright 2019 Centreon (http://www.centreon.com/)
  #
  # Centreon is a full-fledged industry-strength solution that meets
...
$fatpacked{"network/cisco/callmanager/snmp/mode/voicemailusage.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'NETWORK_CISCO_CALLMANAGER_SNMP_MODE_VOICEMAILUSAGE';
  #
  # Copyright 2019 Centreon (http://www.centreon.com/)
  #
  # Centreon is a full-fledged industry-strength solution that meets
...
$fatpacked{"network/cisco/callmanager/snmp/plugin.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'NETWORK_CISCO_CALLMANAGER_SNMP_PLUGIN';
  #
  # Copyright 2019 Centreon (http://www.centreon.com/)
  #
  # Centreon is a full-fledged industry-strength solution that meets
  ...
  package network::cisco::callmanager::snmp::plugin;
  ...
  Check Cisco Call Manager (CCM) in SNMP.
