...
$fatpacked{"apps/protocols/ntp/mode/offset.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'APPS_PROTOCOLS_NTP_MODE_OFFSET';
  #
...
$fatpacked{"apps/protocols/ntp/mode/responsetime.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'APPS_PROTOCOLS_NTP_MODE_RESPONSETIME';
  #
  # Copyright 2019 Centreon (http://www.centreon.com/)
  #
  # Centreon is a full-fledged industry-strength solution that meets
  ...
  package apps::protocols::ntp::mode::responsetime;
  ...
  sub run {
      ...
      eval {
          $Net::NTP::TIMEOUT = $self->{option_results}->{timeout};
          %ntp = get_ntp_response($self->{option_results}->{hostname}, $self->{option_results}->{port});
      };
      if ($@) {
          $self->{output}->output_add(severity => 'CRITICAL',
                                      short_msg => "Couldn't connect to ntp server: " . $@);
...
$fatpacked{"apps/protocols/ntp/plugin.pm"} = '#line '.(1+__LINE__).' "'.__FILE__."\"\n".<<'APPS_PROTOCOLS_NTP_PLUGIN';
  #
  # Copyright 2019 Centreon (http://www.centreon.com/)
  #
  # Centreon is a full-fledged industry-strength solution that meets
  ...
  package apps::protocols::ntp::plugin;
