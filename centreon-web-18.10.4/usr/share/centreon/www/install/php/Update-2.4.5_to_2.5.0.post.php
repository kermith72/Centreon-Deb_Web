...
if (isset($pearDB)) {
    ...
    while ($row = $res->fetchRow()) {
        if (!isset($hc[$row['name']])) {
            ...
            $res2 = $pearDB->query("SELECT MAX(hc_id) as last_id FROM hostcategories WHERE hc_name = '" .
                $pearDB->escape($row['name']) . "'");
            $row2 = $res2->fetchRow();
    ...
    while ($row = $res->fetchRow()) {
        if (!isset($sc[$row['name']])) {
            $pearDB->query("INSERT INTO service_categories (sc_name, sc_description, level, icon_id, sc_activate) " .
                "VALUES (
                    '" . $pearDB->escape($row['name']) . "',
                    '" . $pearDB->escape($row['name']) . "',
                    '" . $pearDB->escape($row['level']) . "',
            ...
            $res2 = $pearDB->query("SELECT MAX(sc_id) as last_id FROM service_categories WHERE sc_name = '" .
                $pearDB->escape($row['name']) . "'");
            $row2 = $res2->fetchRow();
