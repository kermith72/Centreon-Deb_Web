...
if ($form->validate()) {
    ...
    if ($fileObj->isUploadedFile()) {
        ...
        if ($debug) {
            print("/usr/share/centreon/bin/centFillTrapDB -f '".$values["tmp_name"]."' -m ".htmlentities($ret["mnftr"], ENT_QUOTES, "UTF-8")." --severity=info 2>&1");
        }

        $stdout = shell_exec("/usr/share/centreon/bin/centFillTrapDB -f '".$values["tmp_name"]."' -m ".htmlentities($ret["mnftr"], ENT_QUOTES, "UTF-8")." --severity=info 2>&1");
